...
config = get_config()  # call initialize_config() at process entry first
...
| ------------------ | ---- | ------- | ----------- |
| `backend` | String | `"auto"` | Capture backend: `auto` / `opencv` / `picamera2`. `auto` tries OpenCV then Pi CSI |
| `device` | String | `""` | Device path (e.g. `"/dev/video0"`). When set, overrides `camera_index` |
| `camera_index` | Integer | `0` | OpenCV device index (desktop) |
| `frame_width` | Integer | `640` | Preferred frame width (driver may fall back) |
| `frame_height` | Integer | `480` | Preferred frame height |
| `fps` | Integer | `30` | Frame rate (kept in config; photo path is single-frame) |
| `warm_up_frames` | Integer | `5` | Frames to discard after open |
| `Local_VL_url` | String | Zhipu API URL | Visual model API URL |
| `VLapi_key` | String | `""` | Zhipu API key; with URL enables VL analysis |
...
| -------- | ---------------- |
| Desktop USB / built-in | `backend=auto`, use `camera_index` |
| Raspberry Pi USB | `backend=opencv`, set `device` to `/dev/videoX`, user in `video` group |
| Raspberry Pi official CSI | install system `python3-picamera2` (**do not** pip-install picamera2 on macOS), `backend=auto` or `picamera2` |

Implementation: `src/mcp/tools/camera/capture_backend.py`. Details: [Camera Tools](../mcp/camera.md).
...
# In-app: ask verbally "What's in front of the camera?"
```
...
| Configuration Item  | Type    | Default Value | Description                                                  |
| ------------------- | ------- | ------------- | ------------------------------------------------------------ |
| `ENABLED`           | Boolean | false         | Enable built-in WebRTC APM echo cancellation (cancels this app's TTS/music echo) |
| `MUSIC_PARALLEL`    | Boolean | true          | With AEC active, TTS and music play in parallel (music ducked); falls back to pausing music when AEC is bypassed |
| `FRAME_DELAY`       | Integer | 3             | Delay compensation frames; actual compensation = 40ms + N × protocol frame duration |
| `ENABLE_PREPROCESS` | Boolean | true          | Enable noise suppression / high-pass preprocessing           |

### AEC Feature Description

**Echo Cancellation**

- Built-in `libs/webrtc_apm` (WebRTC audio processing), works out of the box on all three platforms with no system configuration
- Uses this app's own TTS/music mix as the reference signal (self far) to cancel echo from the microphone signal
- Automatically bypasses on engine creation failure without affecting normal use
...
python main.py  # Should have no echo and no self-talk when AEC is enabled
...
> 💡 Built-in AEC only cancels audio played by **this app**; to cancel audio from other applications, see the system-level options in [Echo Cancellation](./回声消除.md).
...
At runtime, devices are matched primarily by **name** (IDs change across hot-plug).

### Hot refresh while running (Bluetooth, etc.)

If you connect a Bluetooth headset after the app has started:

1. Confirm the device appears in the OS sound settings as input/output
2. Open **Settings → Audio devices → Refresh devices**  
   (briefly stops mic/playback, reinitializes PortAudio, re-enumerates)
3. Select the device and **Save**  
   → emits `CONFIG_CHANGED`; `AudioPlugin` runs `reload_devices` to switch

If it still does not appear: ensure the OS exposed it as an audio endpoint, or quit and start with the headset already connected (cold start is most reliable).

Implementation: `refresh_portaudio_devices` + event `AUDIO_DEVICES_REFRESH_REQUEST`.
...
## Custom writable directories (cache / logs / music)

**Config files** remain under the user-data `config/` directory (override the whole data root with `XIAOZHI_DATA_DIR`).

Cache, logs, music cache, and keywords can be customized via env vars or `PATHS`. After you change them, the **next startup** **copies** files from the old location into the new one when missing (old directory is kept).

| Use | Env | Config | Default |
|-----|-----|--------|---------|
| User data root | `XIAOZHI_DATA_DIR` | — | platformdirs |
| Cache | `XIAOZHI_CACHE_DIR` | `PATHS.CACHE_DIR` | `{data}/cache` |
| Logs | `XIAOZHI_LOG_DIR` | `PATHS.LOG_DIR` | `{data}/logs` |
| Music | `XIAOZHI_MUSIC_CACHE_DIR` | `PATHS.MUSIC_CACHE_DIR` | `{cache}/music` |
| Keywords | `XIAOZHI_KEYWORDS_DIR` | `PATHS.KEYWORDS_DIR` | `{data}/keywords` |
| MCP plugins | — | `MCP_PLUGINS.DIR` | `{data}/mcp_plugins` |

Priority: **env > PATHS > default**.
