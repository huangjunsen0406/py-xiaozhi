...
**Users who install the official package (DMG / EXE / DEB) do NOT need a system FFmpeg.**

Release builds inject portable `ffmpeg` / `ffprobe` under `libs/ffmpeg/<plat>/<arch>/`. Activation voice prompts use prebaked WAV files and never spawn FFmpeg.

The system dependency steps below are for **source development / running from the repo**.
...
```bash
# Install with Scoop (recommended) — needed for music decode when running from source
...
# Source dev: portaudio / opus required; ffmpeg for music decode (not for activation prompts)
...
| ---- | ---------- | ----- |
| USB UVC | OpenCV (already in project) + `video` group | Set `CAMERA.device=/dev/videoX` or the correct `camera_index` |
| Official CSI module | **System package** `python3-picamera2` | Bookworm libcamera stack; `CAMERA.backend=auto` or `picamera2` |
...
# Official CSI (Pi only — do NOT pip-install picamera2 on macOS)
sudo apt-get install -y python3-picamera2
# If using a venv that must import system packages:
...
> Full config and troubleshooting: [Camera Tools](../mcp/camera.md).
...
3. **System dependencies first**: When developing from source, install system dependencies (ffmpeg, opus, portaudio) before Python dependencies
...
6. **Installer users do not need FFmpeg**: release packages ship portable ffmpeg/ffprobe; only source runs need system FFmpeg or a local `libs/ffmpeg/` inject
7. **Raspberry Pi CSI**: install `python3-picamera2` via apt; do not pip-install picamera2 on macOS/desktop (it depends on Linux-only `python-prctl`)
