...
MCP (Model Context Protocol) lets the AI call local capabilities. Built-in tools are registered with **`register_*_tools(add_tool, …)`**, wired from `McpServer.register_builtin_tools` (called by `McpPlugin.setup`). The global **`@mcp_tool` decorator and auto-discovery have been removed**.
...
Export `register_my_tools` from `__init__.py`, then call it once inside `register_builtin_tools`. Inject runtime services via parameters (closures), not module singletons.
...
Ensure:
1. The package exports `register_*_tools` and it is called from `register_builtin_tools`
