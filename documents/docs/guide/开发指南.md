...
│   │   ├── tooling.py      # McpTool / Property
│   │   └── tools/          # Tools (explicit register_*)
...
MCP (Model Context Protocol) lets the AI call local capabilities. Built-in tools are registered with **`register_*_tools(add_tool, …)`**, wired from `McpServer.add_common_tools` / `McpPlugin.setup`. The global **`@mcp_tool` decorator and auto-discovery have been removed**.

Full walkthrough: [MCP Tool Development Guide](/mcp/).

External plugins (user directory, vendored deps): [External Plugins](/mcp/plugins).

### Creating tools (summary)

```python
# src/mcp/tools/my_tools/register.py
...
def register_my_tools(add_tool: Callable[[McpTool], None]) -> None:
    async def greet(args: dict[str, Any]) -> str:
    ...
    add_tool(
        McpTool(
            "my_tools.greet",
            "Greet the user. Call when they say hello.",
...
Export `register_my_tools` from `__init__.py`, then call it once inside `add_common_tools` (or `McpPlugin.setup`). Inject runtime services via parameters (closures), not module singletons.
...
|--------------|------|
| `STRING` | String |
| `INTEGER` | Optional `min_value` / `max_value` |
| `BOOLEAN` | Optional `default_value` |

Write clear `description` text (what / when / caveats) so the model knows when to call the tool.
...
### Q: MCP tools are not loading?

Ensure:
1. The package exports `register_*_tools` (from `register.py`) and it is called from `add_common_tools` / `McpPlugin.setup`
2. `__init__.py` re-exports the register function
3. Tool names are globally unique; check logs for "registered … MCP tools"
4. Decorator auto-discovery is gone — editing a file alone does not register tools
