...
| 类型 | 依赖 | 说明 |
| ---- | ---- | ---- |
| USB UVC | OpenCV（项目已依赖）+ `video` 组 | 配置 `CAMERA.device=/dev/videoX` 或正确 `camera_index` |
| 官方 CSI 模组 | **系统包** `python3-picamera2` | Bookworm libcamera 栈；`CAMERA.backend=auto` 或 `picamera2` |
...
# 官方 CSI（仅树莓派，不要在 macOS 上 pip 安装 picamera2）
sudo apt-get install -y python3-picamera2
# 若使用 venv 且需 import 系统包：
...
7. **树莓派 CSI**：用 `apt install python3-picamera2`，不要在 macOS / 桌面开发机 pip 安装 picamera2（会依赖仅 Linux 可用的 `python-prctl`）
