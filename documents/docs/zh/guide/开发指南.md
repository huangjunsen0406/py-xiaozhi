...
MCP (Model Context Protocol) 是与 AI 模型交互的工具协议。内置工具通过 **`register_*_tools(add_tool, …)` 显式注册**，由 `McpServer.register_builtin_tools` 统一挂载（`McpPlugin.setup` 只负责调用）；**不再使用** `@mcp_tool` 装饰器与目录自动发现。
...
在 `mcp_server.register_builtin_tools` 中调用：
...
确保：
1. 包导出 `register_*_tools`，并在 `register_builtin_tools` 中显式调用
2. 目录含 `__init__.py` 且从 `_tools` 导出 `register_*`
