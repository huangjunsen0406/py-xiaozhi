...
![四按钮独立按键模块](./images/四按钮独立按键模块.png)

![按钮模块原理图](./images/按钮模块原理图.png)

**硬件接线说明**

按钮模块 6Pin（1 2 3 4 V G）接树莓派 5：

![树莓派5引脚图](./images/树莓派5引脚图.png)
...
![按钮模块产品二维码](./images/按钮模块产品二维码.jpg)
...
MCP (Model Context Protocol) 是与 AI 模型交互的工具协议。内置工具通过 **`register_*_tools(add_tool, …)` 显式注册**，由 `McpServer.add_common_tools` / `McpPlugin.setup` 显式挂载；**不再使用** `@mcp_tool` 装饰器与目录自动发现。

完整步骤与示例见 [MCP 工具开发指南](/zh/mcp/)。

外挂插件（用户目录、自带依赖）见 [MCP 外挂扩展](/zh/mcp/plugins)。

### 创建 MCP 工具（摘要）

在 `src/mcp/tools/<name>/` 中实现：
...
def register_my_tools(add_tool: Callable[[McpTool], None]) -> None:
    ...
    add_tool(
        McpTool(
            "my_tools.greet",
            "向用户打招呼。当用户说'你好'、'Hi'时调用。",
            PropertyList([Property("name", PropertyType.STRING)]),
            greet,
        )
    )
    logger.info("已注册 my_tools")
```

```python
# src/mcp/tools/my_tools/__init__.py
from .register import register_my_tools
__all__ = ["register_my_tools"]
```

在 `mcp_server.add_common_tools`（或 `McpPlugin.setup`）中调用：

```python
from src.mcp.tools.my_tools import register_my_tools
register_my_tools(self.add_tool)
```

需要 `MusicPlayer` / 摄像头等运行时对象时，由容器创建后作为参数传入 `register_*`，闭包持有实例，**不要**写模块级单例。
...
|--------------|------|
| `STRING` | 字符串 |
| `INTEGER` | 整数，可选 `min_value` / `max_value` |
| `BOOLEAN` | 布尔，可选 `default_value` |

### 工具描述最佳实践

`description` 应包含：简短说明、触发话术、行为与注意点，便于模型选择调用时机。
...
config = get_config()  # 入口须先 initialize_config()
...
确保：
1. 包导出 `register_*_tools`（实现于 `register.py`），并在 `add_common_tools` / `McpPlugin.setup` 中显式调用
2. 目录含 `__init__.py`，且 `from .register import register_*_tools`
3. 工具名全局唯一；重启应用后查看日志中的「已注册 … MCP 工具」
4. 生产路径已移除 `@mcp_tool` / 自动发现，仅改文件不会自动加载
