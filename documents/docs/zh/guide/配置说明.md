...
config = get_config()  # 进程入口须先 initialize_config()
...
| 配置项 | 类型 | 默认值 | 说明 |
| ------ | ---- | ------ | ---- |
| `backend` | String | `"auto"` | 采集后端：`auto` / `opencv` / `picamera2`。`auto` 先 OpenCV，失败再试树莓派 CSI |
| `device` | String | `""` | 设备路径（如 `"/dev/video0"`）。非空时优先于 `camera_index`，适合 Linux/Pi USB |
| `camera_index` | Integer | `0` | OpenCV 摄像头索引（桌面常用） |
| `frame_width` | Integer | `640` | 期望画面宽度（驱动可能回落） |
| `frame_height` | Integer | `480` | 期望画面高度 |
| `fps` | Integer | `30` | 帧率（配置保留；当前拍照为单帧采集） |
| `warm_up_frames` | Integer | `5` | 打开后丢弃的预热帧数（USB/Pi 前几帧常无效） |
| `Local_VL_url` | String | 智谱 API 地址 | 视觉模型 API 地址 |
| `VLapi_key` | String | `""` | 智谱 API 密钥；与 URL 都配置时走 VL 分析 |
...
| 场景 | 建议 |
| ---- | ---- |
| 电脑 USB / 内置摄像头 | `backend=auto`，用 `camera_index` |
| 树莓派 USB 摄像头 | `backend=opencv`，`device` 设为 `/dev/videoX`，用户加入 `video` 组 |
| 树莓派官方 CSI | 系统安装 `python3-picamera2`（**不要**在 macOS 上 pip 装 picamera2），`backend=auto` 或 `picamera2` |

实现见 `src/mcp/tools/camera/capture_backend.py`；详细说明见 [摄像头工具](../mcp/camera.md)。
...
| 配置项              | 类型    | 默认值 | 说明                                                         |
| ------------------- | ------- | ------ | ------------------------------------------------------------ |
| `ENABLED`           | Boolean | false  | 启用内置 WebRTC APM 回声消除（消除本应用 TTS/音乐的回声）    |
| `MUSIC_PARALLEL`    | Boolean | true   | AEC 在位时 TTS 与音乐并行播放（音乐闪避）；AEC 旁路时自动回退为暂停音乐 |
| `FRAME_DELAY`       | Integer | 3      | 延迟补偿帧数，实际补偿 = 40ms + N × 协议帧长                 |
| `ENABLE_PREPROCESS` | Boolean | true   | 启用噪声抑制/高通预处理                                      |

### AEC功能说明

**回声消除 (Echo Cancellation)**

- 内置 `libs/webrtc_apm`（WebRTC 音频处理），三端开箱即用，无需系统配置
- 以本应用播放的 TTS/音乐混音作为参考信号（Self far），从麦克风信号中消除回声
- 引擎创建失败时自动旁路，不影响正常使用
...
# 测试回声消除效果
# 1. 启用AEC，播放音乐同时说话
python main.py  # AEC启用时应无回声，且不会自言自语
...
> 💡 内置 AEC 仅消除**本应用**播放的声音；如需消除其它软件的外放声音，参见[回声消除](./回声消除.md)中的系统级方案。
...
运行时优先按 **设备名称** 匹配（ID 会随热插拔变化）。

### 运行中热刷新（蓝牙等）

程序已启动后再连接蓝牙耳机时：

1. 系统声音设置中确认设备已连接并可作输入/输出
2. 打开 **参数设置 → 音频设备 → 刷新设备**  
   （会短暂停麦/播放，并重初始化 PortAudio 后重新枚举）
3. 在列表中选中目标设备并 **保存配置**  
   → 触发 `CONFIG_CHANGED`，`AudioPlugin` 会 `reload_devices` 切到新设备

若刷新后仍没有该设备：确认系统层已暴露音频端点，或退出后「先连蓝牙再启动」（冷启动枚举最稳）。

实现：`refresh_portaudio_devices` + 事件 `AUDIO_DEVICES_REFRESH_REQUEST`。
...
**配置文件**仍在用户数据目录下的 `config/config.json`（可用 `XIAOZHI_DATA_DIR` 改变整个数据根）。

缓存、日志、音乐缓存、唤醒词目录可通过环境变量或配置 `PATHS` 自定义。修改并保存后，**下次启动**会将旧目录中尚未出现在新目录的文件**复制**到新路径（不删除旧目录）。

| 用途 | 环境变量 | 配置项 | 默认 |
|------|----------|--------|------|
| 用户数据根（含 config） | `XIAOZHI_DATA_DIR` | — | 系统用户数据目录/`py-xiaozhi` |
| 缓存 | `XIAOZHI_CACHE_DIR` | `PATHS.CACHE_DIR` | `{数据根}/cache` |
| 日志 | `XIAOZHI_LOG_DIR` | `PATHS.LOG_DIR` | `{数据根}/logs` |
| 音乐缓存 | `XIAOZHI_MUSIC_CACHE_DIR` | `PATHS.MUSIC_CACHE_DIR` | `{缓存}/music` |
| 唤醒词 | `XIAOZHI_KEYWORDS_DIR` | `PATHS.KEYWORDS_DIR` | `{数据根}/keywords` |
| MCP 插件 | — | `MCP_PLUGINS.DIR` | `{数据根}/mcp_plugins` |

优先级：**环境变量 > 配置 PATHS > 默认**。

```json
"PATHS": {
  "CACHE_DIR": "/Volumes/Data/xiaozhi-cache",
  "LOG_DIR": "/Volumes/Data/xiaozhi-logs",
  "MUSIC_CACHE_DIR": null,
  "KEYWORDS_DIR": null
}
```
