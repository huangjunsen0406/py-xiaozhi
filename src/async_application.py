...
class AsyncApplication:
    ...
    _instance: Optional['AsyncApplication'] = None

    @classmethod
    def get_instance(cls) -> 'AsyncApplication':
        """获取单例实例"""
        if cls._instance is None:
            logger.debug("创建AsyncApplication单例实例")
            cls._instance = AsyncApplication()
        return cls._instance

    def __init__(self):
        """初始化应用程序"""
        if AsyncApplication._instance is not None:
            raise Exception("AsyncApplication是单例类，请使用get_instance()获取实例")
        AsyncApplication._instance = self

        logger.debug("初始化AsyncApplication实例")
        ...
        logger.debug("AsyncApplication实例初始化完成")
    ...
    async def run(self, **kwargs):
        ...
        try:
            ...
            self.running = True
            logger.info("AsyncApplication启动完成")
    ...
    async def _send_text_tts(self, text: str):
        ...
        try:
            ...
            logger.error(f"发送文本到TTS时出错: {e}", exc_info=True)
            return False
